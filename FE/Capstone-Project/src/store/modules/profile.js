  ...
  mutations: {
    ...
    UPDATE_PROFILE(state, updatedProfile) {
      ...
      if (
        ...
        state.currentProfile.id === updatedProfile.id
      ) {
  ...
  actions: {
    async fetchProfiles({ commit }, filters = {}) {
      try {
        ...
        // Xử lý pagination response: { count, next, previous, results }
        const profilesData = response.data?.results || response.data || [];
        commit("SET_PROFILES", profilesData);
        return response;
      } catch (error) {
    ...
    async fetchSuggestedProfiles({ commit }, profileId) {
      try {
        commit("SET_LOADING", true);
        commit("SET_ERROR", null);
        const suggestedProfiles = await profileService.getSuggestedProfiles(
          profileId
        );
